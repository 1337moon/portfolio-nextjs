...
const getColorForIndex = (
    ...
    // case 1: global "all" color
    if (colors["all"]) return colors["all"];

    for (const key in colors) {
        if (key.includes("-")) {
            // case 2: range color e.g. "2-6"
        ...
        // ✅ check for "index+" first
            ...
            if (!Number.isNaN(numKey) && index >= numKey) {
                return colors[key];
            }
        }
        else {
...
const BlurText: React.FC<BlurTextProps> = ({
    ...
    const defaultFrom = useMemo(
        () =>
            direction === "right"
                ? { filter: "blur(10px)", opacity: 0, x: -30 }
                : { filter: "blur(10px)", opacity: 0, x: 30 },
        [direction]
    );
    ...
    return (
        <h2
            ref={ref}
            className={`${titleClass} ${className} bg-gradient-to-b from-[#79E2F0] to-[#07545E] bg-clip-text text-transparent font-primary leading-[1.3]`}
